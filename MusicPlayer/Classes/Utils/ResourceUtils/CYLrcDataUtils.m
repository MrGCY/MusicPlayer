...
+ (NSArray<CYLrcModel *> *)getLrcModelsWithFileName:(NSString *)fileName
{
    ...
    for (NSString *timeAndText in timeAndTextArray) {
        ...
        // 创建歌词数据模型并且赋值
        CYLrcModel *lrcModel = [[CYLrcModel alloc] init];
        // 歌词时间
        lrcModel.beginTime = [CYTimeUtils getTimeIntervalWithFormatTime:beginTimeStr];
        // 歌词内容
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        CYLrcModel *nowModel = lrcModels[i];
        CYLrcModel *nextModel = lrcModels[i + 1];
